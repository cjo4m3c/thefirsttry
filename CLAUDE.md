...
- **特殊 L4 後綴**：開始 `-0` / 結束 `-99` / 閘道 `_g` 或 `_g\d+` / 子流程調用 `_s` 或 `_s\d+` / 外部關係人互動 `_e` 或 `_e\d+`（前綴必為既有 L4 任務或 `-0` 開始事件）
- **`_g` / `_s` / `_e` 共用 anchor**：三者都不佔順號，連續計數器互不重置（規格範例 `_s1 → _g → _s2`、`_e1 → _g → _e2`）
...
- Excel parser 寬鬆（容忍點分隔避免解析斷裂）；`validateNumbering` 強制 dash-only + `_g` / `_s` / `_e` 前綴對應，列出所有錯誤列
- 舊 localStorage 點分隔資料在 `storage.normalizeNumber` 載入時自動轉橫線；舊閘道缺 `_g` / 舊子流程缺 `_s` / 舊外部互動缺 `_e` 自動補（`migrateGatewaySuffix` / `migrateSubprocessSuffix` / `migrateInteractionSuffix`）
- **流程圖顯示分層（2026-04-30 後段）**：流程圖只顯示 L4 任務編號；`-0` / `-99` / `_g*` / `_s*` / `_e*` 全部隱藏。編輯器 + 表格顯示完整編號（含後綴）— SOT 是 `displayLabels`，hide 在 `TasksLayer.jsx`
