...
- **特殊類型 L4 後綴**：
  ...
  - **閘道判定範圍（僅以下關鍵字視為獨立閘道元件，需要 `_g` 尾碼）**：`條件分支至`、`並行分支至`、`條件合併來自多個分支`、`並行合併來自`
  - **迴圈返回**（新 `迴圈返回至 X` / 舊 `若未通過則返回 X，若通過則序列流向 Y`）**不是獨立閘道元件**，解析為一般任務 + 往前任務的 back-edge（合併進 `nextTaskIds`），保持矩形圖示，L4 編號維持一般格式
