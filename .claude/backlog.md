...
- ~~**錯落排列 toggle**~~ — DROPPED 2026-05-12（使用者：「測試後確定不要錯落了」。底層 staggerLanes 邏輯一併移除、不再保留入口）
...
`excelImport.js` **31KB**（PR-D5/D10/D12 撐爆，最急）/ `Dashboard.jsx` 26KB。已解：`HelpPanel.jsx` 26KB → 11.3KB（PR #84）/ `taskDefs.js` 17.4KB → 14.3KB（PR #81）/ `c13.js` 17.7KB → c13a 9.1KB + c13b 8.7KB（PR-1 拆檔輪 2026-05-11）/ `storage.js` 15.5KB → 5.9KB + storage/migrations.js 9.9KB（PR-2 拆檔輪 2026-05-11）/ `ContextMenu/index.jsx` 17.5KB → 13.6KB + useContextMenuPosition.js 5.8KB（PR-3 拆檔輪 2026-05-11）
...
- **PR #196（routing 升級 ship）**：col topological+L4 walk / Phase 1 4-pass mixed priority / Phase 3f L1 retry 從 preview toggle 升格成預設行為；移除「改善排版」按鈕；docs/business-spec.md §5 重寫為 routing 規則 SOT（8 個小節）；刪除 `.claude/routing-handover.md`（內容搬進 spec）；HANDOVER.md §2.5 / CLAUDE.md pointer 同步。**對應 backlog #2 Phase A + B 完成**
