...
**使用者規則（2026-04-27 立）**：「太大或推太久試過兩次都 timeout 的，下一次直接給手工步驟」。一個檔案在某次 session timeout 過 2 次（或 size 落在 >15KB），就視同永久走手工，**不要再試 MCP 浪費時間**。即使邊界（如 16KB）這次能通過，下一個 session 仍按 size 規則保守走手工。
...
| **換行黏接**（cosmetic 但醜）：使用者貼上時兩行內容黏成一行，或 `},` 跟下一個物件 `{` 黏住。常見原因是貼上時最末行的換行被吃掉。範例（2026-04-27 OR PR）：FlowEditor.jsx 兩行 `// 4.` 註解黏成一行；HelpPanel.jsx `detail: '…',  },` 跟下一個 `{ tier: 'import' }` 黏住 | Before / After 片段每段尾巴**多留一個明確的空行 + boundary marker**（例如 `};` / 下一個 `function` 開頭）。指引最後加註「⚠️ 比對貼上後文件，每個 `},` 後面必須有換行才接下一個 `{`」。Build / runtime 通常不會因此壞，但若被使用者察覺到會被回報，列入下次一起改 |
...
## 🚨 paste-bundle 期間禁用 `git reset --hard`（2026-04-28 教訓）

paste-bundle 進行中**嚴禁** `git reset --hard origin/<branch>` 同步本地，因為它會直接丟掉所有「本地有但 remote 還沒有」的 commit — 包括還沒 paste 完的 WIP commit、working tree 也會跟著回退。**這個 session 內踩到 2 次**，靠 reflog 撿回。

**錯誤情境（複習）**：
1. 本地 1 個 commit 改了 7 個檔案
2. MCP 推了 3 個小檔 → remote 多 3 個 commit
3. `git reset --hard origin/<branch>` ← ❌ 本地那個 7 檔 commit 整個被 discard，剩下 4 個還沒 paste 的檔案內容也消失
4. 還好 `git reflog` 看得到那個 commit、用 `git checkout <SHA> -- <file>` 還原到 working tree
...
`rebase` 的好處：
- 自動偵測 remote 已有相同內容的 commit 並 skip（顯示 `skipped previously applied commit`）
- 本地還沒 push 的 WIP commit 會留在 HEAD 之上
- 不動 working tree 上未 commit 的編輯

**只有「整輪 paste-bundle 全部完成、檔案 100% 都在 remote」時**才能用 `reset --hard origin/<branch>` 收尾。中間任何一輪都用 rebase。

**事故復原 SOP**：
1. 立刻 `git reflog | head -10` 找最近的 commit SHA
2. `git checkout <SHA> -- <path1> <path2>` 把丟失的檔案內容還原到 working tree
3. `git add -A && git commit -m "WIP: restore lost paste-bundle changes"` 重建 safety net commit
4. 繼續 paste-bundle 流程
## 2026-04-27 更新：使用者規則 + 換行黏接 pattern

**規則升級**：使用者明說「太大或推太久試過兩次都 timeout 的，下一次直接給手工步驟」（見 §1）。實作上：
- 一個檔案在 session 內 timeout 過 2 次 → 立刻轉手工，後續不再試
- size 落 >15KB → 永遠手工（即使這次能通過）
- 邊界 7-15KB 仍可試 MCP，但失敗就轉手工，不要連試 3 次

**新 pattern：換行黏接**（OR 閘道 PR 觀察到，見 §4 表）。使用者貼上時最末行的換行偶爾被吃掉，造成兩行內容黏在一起。範例：
- `FlowEditor.jsx`：兩行 `// 4.` 註解被擠成一行
- `HelpPanel.jsx`：`detail: '…',  },` 跟下個物件 `{` 黏住

Build / runtime 通常不壞（`//` 還是合法註解、`,},` 解析 OK），但會在使用者下次看程式碼時被察覺。預防：
1. Before / After 片段尾巴留**明確空行 + boundary marker**（例 `};` 或下一個 `function` 開頭）
2. 指引尾段加註「⚠️ 比對貼上後文件，每個 `},` 後面必須有換行才接下一個 `{`」
3. 若仍發生，列入「下次一起改」，不必為 cosmetic 開單獨 PR
