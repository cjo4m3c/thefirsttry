...
// rule 用 string[] 列點呈現（HelpPanel 透過 Content 元件 render 成 <ul>）；
// 一律不放反引號 / Markdown code fence —— 規格文字直接以中文敘述。
export const NUMBERING = [
  {
    kind: 'L3 活動編號',
    rule: [
      '三段數字格式：數字-數字-數字（恰好 3 段）',
      '僅接受「-」分隔，不接受「.」分隔',
    ],
    example: ['1-1-5', '5-4-11'],
  },
  {
    kind: 'L4 任務編號',
    rule: [
      '格式：L3 編號 + 「-」+ 順號',
      '只對一般任務從 1 流水計數（開始 / 結束 / 閘道 / 子流程 / 外部互動都不佔順號）',
    ],
    example: ['1-1-5-1', '1-1-5-2', '1-1-5-3'],
  },
  {
    kind: '開始事件',
    rule: ['尾碼必為 0'],
  ...
  },
  {
    kind: '結束事件（多個）',
    rule: [
      '兩個以上結束事件時，依序加 _x1、_x2、_x3…（從 1 開始）',
      '對齊外部 BPMN 連線 / Excel 公式規則（2026-05-13）',
      '單一結束時不加 _x（保持 1-1-5-99）',
    ],
    example: ['1-1-5-99_x1', '1-1-5-99_x2'],
  },
  {
    kind: '閘道（XOR / AND / OR）',
    rule: [
      '單一閘道：前一任務編號 + _g',
      '連續多個：用 _g1、_g2、_g3…',
      '前綴必對應既有 L4 任務或「-0」開始事件',
    ],
    example: ['1-1-5-2_g', '1-1-5-0_g（接在開始之後）'],
  },
  {
    kind: '迴圈返回',
    rule: [
      '舊資料兼容（編輯器不再提供新增）',
      '以任務本身編號 + nextTaskIds = [前面任務]',
    ],
    example: ['1-1-5-4（既有資料保留）'],
  },
  {
    kind: '子流程調用',
    rule: [
      ...
      '連續多個：用 _s1、_s2…',
      '不佔順號',
      '圖上元件顯示所調用的 L3 編號（取代本任務 L4）',
    ],
    example: ['1-1-5-2_s', '1-1-5-2_s1 → _g → _s2（中間閘道不打斷連續性）'],
  },
  {
    kind: '外部關係人互動',
    rule: [
      '單一互動：前一任務編號 + _e',
      '連續多個：用 _e1、_e2…',
      '不佔順號，跟 _g / _s 共用 anchor、計數器互不重置（_e1 → _g → _e2 仍連續）',
      '流程圖上不顯示編號（編輯器 + 表格仍顯示）',
    ],
    example: ['1-1-5-2_e', '1-1-5-2_e1 → _g → _e2'],
  },
];
...
export const ELEMENTS = [
  ...
  },
  {
    ...
    shape: '圓形（實心深色）',
    color: '#111827 填色',
    purpose: '流程的終點，每張圖至少有一個。L4 編號尾碼必為 `99`（單一範例 `1-1-5-99`；多個結束事件依序用 `1-1-5-99_x1`、`1-1-5-99_x2`，2026-05-13）。',
  },
  {
    type: 'L4 任務',
    shape: '圓角矩形',
  ...
  },
  {
    ...
    purpose: [
      '外觀與 L4 任務相同但底色為灰色',
      '外部角色泳道**只能**使用此元件（移到外部泳道時自動 task→interaction）',
      '內部角色泳道**禁止**使用此元件 — 違規元件會在流程圖 / 表格上亮紅色邊框（介面 warning，不影響 PNG / drawio 匯出）',
      '編號用 `_e` 後綴（`1-1-5-2_e`），不佔流水順號',
...
export const VALIDATION = [
  ...
  },
  {
    tier: 'warning',
    rule: 'L4 任務名稱未填寫',
    detail: 'L4 任務（type=task）的 name 為空時跳 warning（純提醒、不擋儲存）。Warning 文字格式 `元件類型 + L4 編號`（例：`L4 任務 1-1-5-3：名稱未填寫`），可幫使用者快速找到對應元件。其他元件類型有自己的命名慣例（閘道有 `[XX閘道]` 前綴、開始 / 結束事件創建時補 `[開始事件]` / `[結束事件]` 前綴），所以這條 rule 不檢查它們。',
  },
  {
    tier: 'warning',
    rule: '空泳道（沒有任何元件）',
  ...
  {
    tier: 'import',
    rule: '角色類型自動推導（Excel 不再讀「角色類型」欄）',
    detail: [
      '匯入時依該角色名下 row 的元件分布推導 `role.type`',
      '全是外部互動（`_e`）→ 外部角色（自動套「[外部角色]」前綴）',
      '全是 L4 任務 → 內部角色',
      '混用 task + `_e` → 內部角色（`_e` row 流程圖 / 表格上紅框警示讓使用者修）',
      '只有開始 / 結束 / 閘道 / L3 活動（無法分辨）→ 預設內部角色',
    ],
  },
  {
    tier: 'import',
  ...
  {
    tier: 'import',
    rule: '外部關係人互動 L4 編號必須以 `_e` 結尾',
    detail: [
      '外部互動的 L4 編號必須是 `前一任務編號_e`（單一）或 `_e1`、`_e2`、`_e3`…（連續多個）',
      '不佔順號',
      'Excel 匯入偵測到 `_e` 尾碼會自動 set `shapeType=interaction`',
  ...
  },
  {
    ...
    detail: [
      '`X_g`、`X_s`、`X_e` 的前綴 `X` 必須是同一份 Excel 中存在的 L4 任務',
      '例外：若是流程的第一個元素，前綴可為 `-0` 開始事件（例 `1-1-5-0_g`）',
      '`_g`、`_s`、`_e` 三者共用 anchor、計數器互不重置（`_e1 → _g → _e2` 仍視為連續）',
    ],
  },
  {
    tier: 'import',
...
export const EDITABLE_ACTIONS = [
  ...
  },
  {
    title: '線段條件文字自動換行',
    desc: [
      '連線上的條件文字（如閘道分支「已核准」/「需重新審核」）會根據線段水平段的可用長度自動分行',
      '相鄰列的短線段：每行 3-4 字，避免溢出到旁邊任務矩形',
      '跨多列的長線段：每行可達 12 字，少分行讀起來連貫',
      '超過 24 字的超長文字自動截斷加 …（建議簡化文字）',
      '每行各有白底襯墊，連線穿過時仍清楚可讀',
    ],
  },
  {
    title: '▦ 流程圖密度切換：緊密 / 預設 / 寬鬆',
    desc: [
      'Header 「▦ 預設」按鈕點一下循環切：⊟ 緊密（0.85x）→ ⊞ 寬鬆（1.15x）→ ▦ 預設（1.0x）',
      '緊密：單頁多看 ~40% 內容，俯瞰大流程結構',
      '寬鬆：放大 115% 字 + 元件，投影 / 共享給客戶看清楚',
      '透過 CSS `zoom` 一次縮放整張流程圖（含字、元件、連線、tooltip）',
      '個人偏好寫 localStorage（重整網頁仍記得）',
      'PNG 匯出時自動還原 1x，下載檔永遠完整解析度（不受密度設定影響）',
      'drawio / Excel 匯出純資料層，本來就不受影響',
    ],
  },
  {
  ...
  {
    title: '▲ ▼ 排序任務',
    desc: '在右側「編輯」面板的「設定流程」分頁，點任務左側的 ▲ ▼ 按鈕改變順序（最上 / 最下會自動 disabled）。順序改了之後 L4 編號自動重排（除非曾經 Excel 匯入帶入特定編號）。',
  },
  {
    title: '▲ ▼ 排序泳道角色',
  ...
  },
  {
    ...
    desc: [
      ...
      '**閘道自動展開分支編輯**：點選閘道時，選單會自動展開「編輯閘道」分支條件區，且該按鈕排在動作列最頂端（branch 編輯是閘道最常見動作）',
    ],
  },
  {
    title: '新增閘道：允許先新增再補分支',
    desc: [
      'tooltip / 編輯器「新增閘道」表單預設 2 條空分支，使用者可填可不填 target',
      '即使所有分支都還沒指定目標也可以按「確定」建立閘道（不再卡 disabled）',
      '建立後分支沒指 target → 儲存時跳 warning「閘道應有至少 2 條分支」/「節點必須設定下一步」',
      '使用者可後續點閘道 → 自動展開的「編輯閘道」區塊補上 target',
    ],
  },
  {
    title: 'Hover 任務看重點說明',
...
export const FORBIDDEN_RULES = [
  ...
  },
  {
    ...
    desc: [
      ...
      '外部關係人互動 `1-1-1-X_e`（單一 _e；連續多個用 _e1、_e2…）',
      '僅接受橫線分隔（舊資料用「.」載入時自動轉）',
      '詳見上方「驗證規則 → Import」段落',
    ],
  },
