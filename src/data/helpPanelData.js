...
export const ELEMENTS = [
  ...
  },
  {
    ...
    shape: '圓角矩形（灰底）',
    color: '#A0A0A0 / 灰框',
    purpose: '外觀與 L4 任務相同但底色為灰色。**規則（2026-04-30 後段更新）**：(a) 外部角色泳道**強制**使用此元件（移到外部泳道時 task→interaction，不可逆）(b) 內部角色泳道**允許**使用此元件，但儲存時跳 warning 讓使用者檢查（移到內部時保留 shape）(c) 編號用 `_w` 後綴（`1-1-5-2_w`），不佔順號 (d) 流程圖上不顯示編號，編輯器 + 表格仍完整顯示',
  },
  {
    type: 'L3 活動（子流程調用）',
    shape: '書端矩形（左右側垂直分隔線）',
...
export const VALIDATION = [
  ...
  },
  {
    tier: 'warning',
    rule: '外部互動建議放外部角色泳道',
    detail: '外部關係人互動元件（`shapeType=interaction`）放在內部角色泳道時跳 warning，使用者可選「仍然儲存」。外部角色泳道強制使用此元件、移過去自動轉換；內部角色泳道允許但建議檢查是否錯放（外部互動 = 跨組織 / 客戶 / 系統互動，通常該歸外部角色）。',
  },
  {
    tier: 'import',
    rule: '條件分支標籤可選填',
