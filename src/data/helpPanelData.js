...
export const ELEMENTS = [
  ...
  },
  {
    ...
    shape: '菱形（內含 ○ 符號）',
    color: '#FEF9C3 / 黃框',
    purpose: '包容分支：每個條件獨立評估，凡為真者建立並行路徑（可同時觸發 1~N 條）。編號規則同 XOR。Excel 標記：`包容分支至 X（A）、Y（B）` 或 `可能分支至 …`（兩種動詞都吃）。合併目標自動產生「包容合併 X、Y，序列流向 Z」。',
  },
];
...
export const VALIDATION = [
  ...
  },
  {
    tier: 'warning',
    rule: '非結束節點必須設定下一步',
    detail: '流程設定為「序列流向」、「條件分支」、「並行分支」、「包容分支」、「子流程調用」、「迴圈返回」、「流程開始」的節點，都應該設定至少一個有效目標。未設定時跳 warning modal。',
  },
  {
    tier: 'warning',
    rule: '合併目標自動偵測（不需手動標記）',
    detail: '當任意任務（含閘道）收到 ≥2 個其他任務指向，系統自動在「任務關聯說明」前面插入「並行/條件/包容合併 X、Y，序列流向 Z」。合併類型由上游閘道種類推斷，混合來源或一般任務 source 預設為條件合併。PR-B 2026-04-29 起編輯器選單不再有 parallel-merge / conditional-merge / inclusive-merge 選項。',
  },
  {
    tier: 'warning',
...
export const CONNECTIONS = [
  ...
  {
    title: '合併（merge）= 自動偵測，不在編輯器選單中',
    desc: '**合併不是「流程設定」選項**。當任意任務（含閘道）收到 ≥2 條 incoming 連線時，系統自動在「任務關聯說明」產生合併文字，列出所有來源任務編號 + 序列流向。範例：`並行合併 5-1-3-3、5-1-3-4，序列流向 5-1-3-6`。合併類型由上游 source 的閘道種類推斷（AND→並行、OR→包容、XOR/混合/一般任務→條件，預設）。',
  },
  {
    title: '流程開始',
