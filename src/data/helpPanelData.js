...
export const ELEMENTS = [
  ...
  },
  {
    ...
    purpose: [
      ...
      '預設 2 條分支，可自行新增 N 條（≥2，否則儲存跳 warning）',
      '**每條分支必須填寫條件標籤**（觸發條件），未填儲存時跳 warning',
      'Excel 標記：`條件分支至 …`',
    ],
  },
  {
...
export const VALIDATION = [
  ...
  },
  {
    ...
    rule: '排他閘道分支必須填寫條件標籤',
    detail: [
      '排他閘道（XOR）每條分支的「條件標籤」是觸發條件，必須填寫',
      '留空時儲存跳 warning「有 N 個分支沒有填寫條件標籤」',
      '並行閘道（AND）/ 包容閘道（OR）的標籤**選填**，留空不跳 warning',
      '原因：XOR 是「擇一執行」需要明確判斷條件；AND「全部執行不評估條件」、OR「獨立評估」標籤可作註記用',
    ],
  },
  {
    tier: 'warning',
...
export const EDITABLE_ACTIONS = [
  {
    title: '儲存提醒：「儲存」按鈕黃色高亮 + 脈衝動畫 + 慶祝回應',
    desc: [
      '兩種情境會把儲存按鈕變成 **黃色實心 + 脈衝動畫**，提醒使用者該存檔了',
      '**情境 1（短暫提醒）**：持續編輯超過 3 分鐘 / 5 分鐘的時間點，按鈕脈衝 8 秒後自動停（不持續干擾）',
      '**情境 2（持續閃耀）**：閒置 ≥1.5 分鐘且有未儲存變更時，按鈕持續脈衝直到使用者繼續編輯或儲存',
      '繼續編輯會自動取消「持續閃耀」並重設閒置計時',
      'hover 儲存按鈕看到的 tooltip 是隨機鼓勵句子（每次編輯 session 隨機抽一句）',
      '按下儲存成功後：按鈕短暫變綠閃光 + ✨ 飄字慶祝動畫（700~900ms），同時 logo 揮手',
    ],
  },
  {
  ...
  },
  {
    ...
    desc: [
      ...
      '新增的元件會自動接好連線避免孤兒',
      '**選單可拖曳**：左上角 ☰ 圖標可拖到任何位置（避開原始任務 / 找空白區域）',
      '**自動防溢出**：展開「轉換為」/「新增其他」等子表單時，選單若會超出視窗邊界自動推回；超過 70% 視窗高度時內部出現捲軸（header 永遠可見）',
    ],
  },
  {
    title: 'Hover 任務看重點說明',
