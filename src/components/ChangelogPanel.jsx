...
const CHANGELOG = [
  {
    date: '2026-04-21',
    title: 'Excel 上傳：閘道鏈完整性警告（不擋匯入）',
    items: [
      '新增 soft validator：檢查每個 `X_g` / `X_g{n}` 閘道的前置元件是否真的連線指向它',
      '規則：`X_g` 或 `X_g1` 的前置是 base 任務 X；`X_g{n}` (n≥2) 的前置是 `X_g{n-1}`',
      '若前置元件不存在或任務關聯說明未指向該閘道，Excel 仍可匯入，但首頁跳出**黃色警告橫條**列出所有違規列',
      '訊息範例：「第 4 列閘道 1-1-1-1_g：前置元件 1-1-1-1（第 3 列）的任務關聯說明未指向 1-1-1-1_g」',
    ],
  },
  {
    date: '2026-04-21',
