...
export function TasksLayer({ tasks, positions, l4Numbers, hoveredId, hoveredConnEndpoints,
  ...
  return tasks.map(task => {
    ...
    // `_e*`, `-0`, `-99`, `-99_x*`) — they're still visible in the editor
    // + table. L3 activity (subprocess call) shows the called L3 number.
    let num = l4Numbers[task.id];
    if (num && /(_g\d*|_s\d*|_e\d*|-0|-99(_x\d+)?)$/.test(num)) num = undefined;
