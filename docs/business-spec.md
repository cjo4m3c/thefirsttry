...
| 結束事件 L4（單一） | 尾碼必為 `99` | `1-1-5-99` |
| 結束事件 L4（多個） | `${L3}-99_x1`、`-99_x2`、`-99_x3`… **（從 1 開始）** | `1-1-5-99_x1`、`1-1-5-99_x2` |
...
- 舊多結束事件缺 `_x` 後綴 / `_x` 跳號（例 `_x3` 但只有 2 個）載入時自動補正（`storage.migrateEndSuffix`，2026-05-13），補正完成後寫回 localStorage 並在 Dashboard banner 顯示「🔧 結束事件編號已自動更新」訊息一次

### 2.7 對應實作

- **格式驗證 regex 的單一來源**：`src/utils/`（`taskDefs` 內 7 個常數）
  - `L3_NUMBER_PATTERN`、`L4_NUMBER_PATTERN`（含 `_g` / `_s` / `_e` / `-99_x` 變體）、`L4_START_PATTERN`、`L4_END_PATTERN`（含 `_x` 變體）、`L4_END_X_PATTERN`、`L4_GATEWAY_PATTERN`、`L4_SUBPROCESS_PATTERN`
- **Excel 匯入 parser**：`src/utils/`（`excelImport` 寬鬆吃新舊格式 + `validateNumbering` 強制 `_s` / `_g` / `_x` 規則）
- **顯示計算**：`src/model/`（`flowSelectors.computeDisplayLabels` 唯一 source of truth；對 `_s` / `_g` 共用 anchor `lastTaskBase`，連續計數器 `gwConsec` / `spConsec` / `intConsec` / `endConsec` 互不重置；end 編號 position-derived 不 honor stored）
- **資料遷移**：`src/utils/`（`storage.normalizeNumber` + `migrateGatewaySuffix` + `migrateSubprocessSuffix` + `migrateInteractionSuffix` + `migrateEndSuffix`）
...
> - `-0` → 開始事件
> - `-99` / `-99_x\d+` → 結束事件（單一用 `-99`；多個用 `-99_x1` / `-99_x2`…，2026-05-13 起拉齊外部 BPMN / Excel 規則）
...
- **流程圖只顯示 L3 / L4 的「正式編號」**
- start (`-0`) / end (`-99` / `-99_x*`) / 閘道 (`_g*`) / 子流程 (`_s*`) / 外部互動 (`_e*`) 編號**不顯示在流程圖上**（僅作辨識用）
...
- **字級** 7 階：`--fs-h1:24` / `--fs-h2:20` / `--fs-h3:18` / `--fs-ui:16` / `--fs-body:14` / `--fs-label:12` / `--fs-caption:11`（最小 11px、不允許更小）。額外 `--fs-display:48` 為裝飾外掛（不計入 7 階核心）。Tailwind 對應：`text-2xl=24` / `text-xl=20` / `text-lg=18` / `text-base=16` / `text-sm=14` / `text-xs=12`
...
- **不用 emoji**（2026-05-19 PR #240 立）：UI 文字、按鈕、標題、空狀態 / banner 訊息一律**不使用 emoji**（如 📋 / ⚠️ / ❌ / ✓ / 🔍 / ⛔ 等）。語意應透過 token 色（warning 黃 / danger 紅 / info 藍）+ 文字傳達。例外：功能性 unicode 符號保留（`✕` 關閉 / `▾` dropdown / `▲▼` 排序 / `↶↷` undo/redo / `→` 流向）。註解內 emoji 不強制（但不鼓勵）。
