...
| 外部關係人互動 L4（單一） | 前一任務編號 + `_w` | `1-1-5-2_w` |
| 外部關係人互動 L4（連續多個） | 前一任務編號 + `_w1`、`_w2`、`_w3`… **（從 1 開始）** | `1-1-5-2_w1`、`1-1-5-2_w2` |
...
| 外部關係人互動 | 圓角矩形 | `#A0A0A0` 灰底 | 外部角色泳道**強制**使用此元件；內部泳道**允許**但儲存跳 warning。L4 編號用 `_w` 後綴（不佔順號），例 `1-1-5-2_w`、連續用 `_w1`/`_w2`。**流程圖不顯示編號**（編輯器 + 表格仍顯示完整含 `_w` 編號）|
...
### 3.1 外部關係人互動 — 不對稱 sync + `_w` 編號（2026-04-30 後段更新）

外部關係人互動 = `shapeType === 'interaction'`，編號用 `_w` 後綴（不佔順號，跟 `_g` / `_s` 同類）。

**Sync 規則（不對稱）**：

| 觸發 | 結果 |
|---|---|
| 任務移到 external 泳道 | `shapeType` 強制 = `interaction`（外部角色「不能用任務」）+ 重新編號為 `_w` |
...
| 載入舊 localStorage 資料 | `storage.migrateInteractionSuffix` 一次性 strip 舊編號讓 computeDisplayLabels 重推 `_w` |
...
**Excel 匯入**：parser 偵測 `_w\d*$` 的 L4 編號 → set `shapeType=interaction`，**不強制翻角色 type**（不 cascade）；若該 row 角色為 internal，validation 3e 跳 warning 讓使用者檢查（仍可儲存）。

**流程圖顯示**：`_w*` 編號**不顯示在流程圖任務元件上**（同 `_g*` / `_s*` / `-0` / `-99` 規則）；編輯器 + 表格仍顯示完整 `_w` 編號（含後綴）。

**對應實作**：`src/utils/elementTypes.js`（`applyRoleChange` / `syncTasksToRoles` / `targetShapeFor` 不對稱 sync）、`src/utils/taskDefs.js`（`L4_INTERACTION_PATTERN`）、`src/model/flowSelectors.js`（`computeDisplayLabels` `_w` derivation）、`src/components/DiagramRenderer/TasksLayer.jsx`（hide regex 含 `_w*`）、`src/utils/storage.js`（`migrateInteractionSuffix`）、`src/utils/excelImport.js`（`isInteraction = /_w\d*$/`）、`src/model/validation.js`（rule 3e）。
