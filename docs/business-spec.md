...
**僅接受 `-`（橫線）分隔**，**不接受 `.`（點）分隔**。閘道有 `_g` 後綴例外。
...
### 2.3 閘道編號的「前綴必對應既有 L4 任務」原則

`X_g` 的前綴 `X` 必須是同一份流程中存在的 L4 任務編號。匯入時若 `X_g` 沒有對應任務，會被擋下。
...
- **格式驗證 regex 的單一來源**：`src/utils/`（`taskDefs` 內 5 個常數）
  - `L3_NUMBER_PATTERN`、`L4_NUMBER_PATTERN`（含 `_g` 變體）、`L4_START_PATTERN`、`L4_END_PATTERN`、`L4_GATEWAY_PATTERN`
- **Excel 匯入 parser**：`src/utils/`（`excelImport` 寬鬆吃新舊格式）
- **顯示計算**：`src/utils/`（`computeDisplayLabels` 唯一 source of truth）
- **資料遷移**：`src/utils/`（`storage.normalizeNumber`）
