...
- **字級** 7 階：`--fs-h1:24` / `--fs-h2:20` / `--fs-h3:18` / `--fs-ui:16` / `--fs-body:14` / `--fs-label:12` / `--fs-caption:11`（最小 11px、不允許更小）。額外 `--fs-display:48` 為裝飾外掛（空狀態大型 emoji），不計入 7 階核心。Tailwind 對應：`text-2xl=24` / `text-xl=20` / `text-lg=18` / `text-base=16` / `text-sm=14` / `text-xs=12`
